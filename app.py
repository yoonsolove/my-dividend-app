...
# 1. 앱 설정
st.set_page_config(page_title="배당 마스터 v15.3", layout="wide", page_icon="📈")
...
# --- [메인 화면 상단: 종목 관리 및 동기화 로직] ---
st.title("📈 배당 마스터 v15.3 (데이터 동기화 수정)")

if not st.session_state.portfolio.empty:
    col_sel, col_del = st.columns([3, 1])
    with col_sel:
        # ⚠️ 중요: 세션 상태에 저장된 현재 선택 종목 인덱스 관리
        selected_stock = st.selectbox(
            "📝 수정할 종목 선택 (선택 시 왼쪽 입력창 즉시 반영):", 
            ["새 종목 추가"] + list(st.session_state.portfolio["종목명"]),
            key="stock_selector"
        )
        
        # 종목을 선택했을 때 세션 데이터를 즉시 업데이트
        if selected_stock != "새 종목 추가":
            t = st.session_state.portfolio[st.session_state.portfolio["종목명"] == selected_stock].iloc[0]
            # 이전 값과 다를 때만 업데이트하고 리런
            if st.session_state.edit_data.get("ticker") != t["종목명"]:
                st.session_state.edit_data = {
                    "ticker": t["종목명"], "count": int(t["보유수량"]), "price": float(t["현재주가"]), 
                    "dps": float(t["주당배당금"]), "growth": float(t["배당성장률"]), 
                    "cat": t["유형"], "cycle": t.get("지급주기", "월배당")
                }
                st.rerun() # 🚀 데이터 동기화를 위해 즉시 다시 그리기
        else:
            if st.session_state.edit_data.get("ticker") != "":
                st.session_state.edit_data = {"ticker": "", "count": 2080, "price": 0.0, "dps": 0.0, "growth": 5.0, "cat": "일반", "cycle": "월배당"}
                st.rerun()
...
if not st.session_state.portfolio.empty:
    ...
    with set_c2: monthly_add = st.number_input("💵 매달 추가 투자금", value=1000000, step=100000)
