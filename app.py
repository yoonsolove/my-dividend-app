...
# 1. 앱 설정
st.set_page_config(page_title="배당 마스터 v10.5", layout="wide", page_icon="🔄")

# 세션 상태 초기화
if 'portfolio' not in st.session_state:
    st.session_state.portfolio = pd.DataFrame(columns=["종목명", "보유수량", "현재주가", "주당배당금", "배당성장률", "유형"])

# 수정 모드를 위한 임시 데이터 저장소
if 'edit_data' not in st.session_state:
    st.session_state.edit_data = {"ticker": "", "count": 0, "price": 0.0, "dps": 0.0, "growth": 0.0, "cat": "일반"}

# --- [메인 화면 상단] 수정할 종목 선택 ---
st.title("📈 배당 성장 시뮬레이터 (데이터 연동형)")

if not st.session_state.portfolio.empty:
    col_sel1, col_sel2 = st.columns([2, 1])
    with col_sel1:
        selected_stock = st.selectbox("📝 수정할 종목을 선택하면 왼쪽 입력창에 자동 반영됩니다:", 
...
if st.sidebar.button("실시간 데이터 불러오기"):
    try:
        with st.spinner('데이터 추출 중...'):
            ...
            st.sidebar.success("분석 완료! 아래 수치를 확인하세요.")
            st.rerun() # 화면 갱신
    except Exception as e:
        st.sidebar.error(f"오류: {e}")

st.sidebar.markdown("---")
st.sidebar.title("✍️ 2단계: 등록 및 수정")
...
if save_btn:
    new_data = pd.DataFrame([[ticker_input, f_count, f_price, f_dps, f_growth, f_cat]], 
                            columns=st.session_state.portfolio.columns)
    # 기존 데이터가 있으면 교체, 없으면 추가
    ...
    st.success(f"{ticker_input} 저장 완료!")
    st.rerun()
...
if st.session_state.portfolio.empty:
    ...
    st.write(f"### 📅 {target_years}개년 예상 월 배당금")
