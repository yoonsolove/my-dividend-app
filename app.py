...
# 1. 앱 설정
st.set_page_config(page_title="통합 배당 관리 시스템", page_icon="💸", layout="wide")
...
# 4. 사이드바 UI (모든 메뉴 통합 및 복구)
st.sidebar.header(f"👤 {st.session_state.get('user_name', '윤재')}님의 설정")
...
# B. 보유 종목 관리 섹션 (복구된 부분!)
st.sidebar.subheader("📦 보유 종목 수정/삭제")
for i, stock in enumerate(st.session_state.stock_list):
    with st.sidebar.expander(f"{stock['name']} ({stock['ticker']})"):
        # 수량 변경
        u_qty = st.number_input(f"수량 변경", value=stock['qty'], key=f"edit_qty_{i}")
        st.session_state.stock_list[i]['qty'] = u_qty
        # 삭제 버튼
        if st.button(f"🗑️ 삭제하기", key=f"delete_btn_{i}"):
...
# C. 물가 및 시뮬레이션 설정
st.sidebar.subheader("🍔 물가 및 미래 설정")
...
for s in st.session_state.stock_list:
    ...
    portfolio_data.append({
        ...
        "자산가치": val, "월배당(세전)": div_pre, "예상세금": tax, "월배당(세후)": div_pre-tax
    })
...
# 7. 상세 내역 및 그래프
t1, t2 = st.tabs(["📋 종목 상세 리스트", "📅 세후 배당 캘린더"])
with t1:
...
st.divider()
st.markdown(f"<center>💖 <b>{user_name} & 소은</b> 통합 관리 v4.9 💖</center>", unsafe_allow_html=True)
