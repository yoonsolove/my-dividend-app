...
# 1. 앱 설정
st.set_page_config(page_title="배당 마스터 v10.6", layout="wide", page_icon="🔄")
...
if save_btn:
    if f_price <= 0:
        ...
        new_data = pd.DataFrame([[ticker_input, f_count, f_price, f_dps, f_growth, f_cat]], columns=st.session_state.portfolio.columns)
...
if st.session_state.portfolio.empty:
    ...
    for _, row in st.session_state.portfolio.iterrows():
        ...
        for y in years:
            ...
            # 🔥 ZeroDivisionError 방지: 주가가 0이면 1로 취급하여 에러 방지
            safe_price = max(c_price, 1.0)
            c_shares += (net_div + (monthly_add * 12 / len(st.session_state.portfolio))) / safe_price
