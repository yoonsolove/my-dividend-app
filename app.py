...
# 1. 앱 설정 (모바일 최적화 레이아웃)
st.set_page_config(page_title="배당 대시보드 v5.0", page_icon="💸", layout="wide")

# 2. 데이터 가져오기 함수 (기존 로직 유지)
...
for s in st.session_state.stock_list:
    ...
    portfolio_data.append({
        ...
        "자산가치": val, "월배당(세전)": div_pre, "예상세금": div_pre * 0.154, "월배당(세후)": div_pre * 0.846
    })
...
# 지표 섹션 (모바일에서 자동으로 2x2 또는 4x1로 정렬됨)
c1, c2, c3, c4 = st.columns([1,1,1,1])
c1.metric("총 자산", f"{total_asset:,.0f} 원")
c2.metric("월 수령액(세후)", f"{total_div_post:,.0f} 원")
c3.metric("월 예상 세금", f"{total_div_pre*0.154:,.0f} 원")
c4.metric("연 예상 세금", f"{total_div_pre*12*0.154:,.0f} 원")

# 치킨/커피 지수 (물가 설정은 사이드바 유지)
chicken_p = st.sidebar.number_input("🍗 치킨 가격", value=30000)
coffee_p = st.sidebar.number_input("☕ 커피 가격", value=5000)
st.info(f"✨ 이번 달 배당금은 **치킨 {total_div_post//chicken_p:,.0f}마리** 가치입니다!")

st.divider()

# 6. [메인 화면 중단] 탭 및 그래프
t1, t2 = st.tabs(["📋 상세 리스트", "📅 배당 캘린더"])
with t1:
    st.dataframe(df.style.format({"자산가치": "{:,.0f}", "월배당(세전)": "{:,.0f}", "예상세금": "{:,.0f}", "월배당(세후)": "{:,.0f}"}), use_container_width=True)
with t2:
    cal_list = []
...
# 7. [메인 화면 하단] 종목 관리 메뉴 (모바일 최적화 포인트!)
st.divider()
st.subheader("🛠️ 포트폴리오 관리")
col_add, col_edit = st.columns(2)

with col_add:
    with st.expander("➕ 새 종목 추가하기"):
...
with col_edit:
    with st.expander("📦 보유 종목 수정/삭제"):
        for i, stock in enumerate(st.session_state.stock_list):
            c_e1, c_e2 = st.columns([3, 1])
            with c_e1:
                new_q = st.number_input(f"{stock['name']} 수량", value=stock['qty'], key=f"m_q_{i}")
                st.session_state.stock_list[i]['qty'] = new_q
            with c_e2:
                if st.button("🗑️", key=f"m_d_{i}"):
...
for m in range(1, (sim_years * 12) + 1):
    ...
    if m % 12 == 0: sim_data.append({"년수": f"{m//12}년", "자산": int(temp_asset)})
st.plotly_chart(px.area(pd.DataFrame(sim_data), x="년수", y="자산"), use_container_width=True)

st.divider()
st.markdown(f"<center>💖 <b>{user_name} & 소은</b> 통합 관리 v5.0 💖</center>", unsafe_allow_html=True)
